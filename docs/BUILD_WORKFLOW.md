...
|----|------|------|-----------|------|--------|-------|
| T0.1 | Snapshot legacy data: fetch the talks TSV (`config.json` → `dataSources.talks`) into `data/legacy/talks.tsv`; probe the Meetup `sig` URL and record the result against HLD Open Q2 | 001/003 | — | snapshot committed; probe result recorded | todo | agent |
...
| T0.3 | Repo layout per ADR-006: create `web/` and `cms/` skeletons; root README note; legacy root untouched | ADR-006 | — | dirs exist; legacy unchanged | todo | agent |
| T0.4 | CI skeleton: GitHub Actions running lint, typecheck, unit tests, and build on PRs | TEST_STRATEGY | T0.3 | workflow green on a no-op run | todo | agent |
